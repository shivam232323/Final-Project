...
exports.applyForJob = async (req, res) => {
   ...
   await pool.query(query, (err, value) => {
      if (err) throw err;
         ...
         if (value[0]) {
            const sql = `select user_id from applied_jobs where job_id  = ${jobId} and user_id = ${userId}`;
...
exports.appliedJobs = async (req, res) => {
   ...
   const sql = `
    select company_name , designation, job_location from job 
    where job_id in(select job_id from applied_jobs where user_id = ${userId});`

   await pool.query(sql, (err, response) => {
      if (err) throw err;
      else {
         if (!response[0]) {
            res.json("You have not applied for any job")
         } else {
